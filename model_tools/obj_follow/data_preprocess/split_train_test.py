...
def generate_label():
    with open(train_label, 'a+') as train_label_f:
        for train_img in os.listdir(train_dir_path):
            if train_img.split('_')[1] == 'danger0':
                train_label_f.write(train_img + ' ' + '0' + '\n')
            elif train_img.split('_')[1] == 'danger1':
                train_label_f.write(train_img + ' ' + '1' + '\n')
            elif train_img.split('_')[1] == 'safe0':
                train_label_f.write(train_img + ' ' + '2' + '\n')
            elif train_img.split('_')[1] == 'safe1':
                train_label_f.write(train_img + ' ' + '3' + '\n')
    train_label_f.close()
    
    with open(test_label, 'a+') as test_label_f:
        for test_img in os.listdir(test_dir_path):
            if test_img.split('_')[1] == 'danger0':
                test_label_f.write(test_img + ' ' + '0' + '\n')
            elif test_img.split('_')[1] == 'danger1':
                test_label_f.write(test_img + ' ' + '1' + '\n')
            elif test_img.split('_')[1] == 'safe0':
                test_label_f.write(test_img + ' ' + '2' + '\n')
            elif test_img.split('_')[1] == 'safe1':
                test_label_f.write(test_img + ' ' + '3' + '\n')
    test_label_f.close()
