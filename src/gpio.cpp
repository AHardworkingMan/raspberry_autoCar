...
int gpio::i2c_1_init()
{
    ...
    if (ioctl(fd, I2C_RETRIES, 1) < 0) {
        close(fd);
        fd = 0;
        ERROR_LOG("set i2c-1 retry fail!\n");
        return -1;
    }
    ...
    if (ioctl(fd, I2C_TIMEOUT, 1) < 0) {
        close(fd);
        fd = 0;
        ERROR_LOG("set i2c-1 timeout fail!\n");
        return -1;
    }
    return 0;
}
...
int gpio::PCA6416_gpio_set_direction(int pin,int dir)
{
    ...
    if((pin!=3)&&(pin!=4)&&(pin!=5)&&(pin!=6)&&(pin!=7)){
        ERROR_LOG("PCA6416 pin not right ,pin param must be 3,4,5,6,7\n");
        return -1;
    }
    ...
    if (ret != 0) {
        close(fd);
        fd = 0;
        ERROR_LOG("GPIO read %#x %#x to %#x fail!\n", slave, data, reg);
        return -1;
    }
    ...
    if (ret != 0) {
        close(fd);
        fd = 0;
        ERROR_LOG("GPIO write %#x %#x to %#x fail!\n", slave, data, reg);
        return -1;
    }
...
int gpio::ASCEND310_gpio_set_direction(int pin,int dir)
{
    ...
    {
        ERROR_LOG("ASCEND310 pin not right ,pin param must be 0,1\n");
        return -1;
    }

    if(-1 == fd_direction)
    {
        ERROR_LOG("open gpio DIR file error pin=%d",pin);
        return(-1);
    }
    ...
    if(dir == 0)
    {
        ...
        {
            ERROR_LOG("gpio write operation error pin=%d",pin);
            close(fd_direction);
            return(-1);
        }
    }
...
int gpio::ASCEND310_gpio_set_value(int pin,int val)
{
    ...
    if(-1 == fd_gpio_value)
    {
        ERROR_LOG("open gpio VAL file error pin=%d",pin);
        return(-1);
    }
...
int gpio::ASCEND310_gpio_get_value(int pin,int *val)
{
    ...
    {
          ERROR_LOG("Failed to read value pin=%d",pin);
          return -1;
